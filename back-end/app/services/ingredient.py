...
class IngredientService:
    async def get_by_id(self, db:SessionDep, id: int) -> Ingredient:
        ingredient_model = ingredient.get_by_id(db=db, id=id)
    ...
    async def update_ingredient(self, db: SessionDep, obj_in: IngredientUpdate) -> Ingredient:
        ingredient_model = await self.get_by_id(db=db, id=obj_in.id)

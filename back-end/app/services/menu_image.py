...
class MenuImageService:
    async def get_menu_image_by_menu_id(self, db: SessionDep, menu_id: int) -> Optional[MenuImage]:
        return menu_image.get_menu_image_by_menu_id(db=db, menu_id=menu_id)
    
    async def is_existed_menu_image_by_menu_id(self, db: SessionDep, menu_id: int) -> bool:
        return bool(menu_image.get_menu_image_by_menu_id(db=db, menu_id=menu_id))

    async def create_menu_image(self, db: SessionDep, obj_in: MenuImageCreate) -> MenuImage:
        if await self.is_existed_menu_image_by_menu_id(db=db, menu_id=obj_in.menu_id):
    ...
    async def update_menu_image(self, db: SessionDep, obj_in: MenuImageUpdate) -> MenuImage:
        menu_image_model = await self.get_menu_image_by_menu_id(db=db, menu_id=obj_in.menu_id)
        if menu_image_model:
